...
def evalcache(datasource):
    ...
    for i, (data, target) in enumerate(datasource):
        ...
        next_word_history = mx.nd.concat([mx.nd.one_hot([t], on_value = 1, off_value = 0) for t in target], dim = 0) if next_word_history is None else mx.nd.concat([next_word_history, mx.nd.one_hot([t], on_value = 1, off_value = 0) for t in target], dim = 0)
        ##cache_history
...
if __name__ == '__main__':
    ...
    val_L = evalcache(val_data)
    test_L = evalcache(test_data)
